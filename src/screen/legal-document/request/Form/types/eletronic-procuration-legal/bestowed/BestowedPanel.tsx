...
import { getEletronicLegal, getEletronicLegalGranteds } from "src/core/store/modules/legal-document-request/selectors";
...
export type BestowedPanelHandler = {
	...
	getExtraData: () => LegalDocumentEletronicProcurationLegalExtraData
}
...
const BestowedPanelComponent: ForwardRefRenderFunction<BestowedPanelHandler, BestowedPanelProps> = (props, ref) => {
	...
	const isNew = id === "novo"
	const granteds = useSelector(getEletronicLegalGranteds)
	const item = useSelector(getEletronicLegal)

	const extraData = useFormik<LegalDocumentEletronicProcurationLegalExtraData>({
		initialValues: {
			justification: "",
			powers: ""
		},
