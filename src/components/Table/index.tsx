			...
			<MuiTable>
				<TableHead>
					<TableRow data-testid="table-row-header">
						...
						{showActionColumn && (
							<TableCell
								component="td"
								/* colSpan={onSelectRadio ? 2 : 1}
								width={ quantActions > 2 ? sizeActions[3] : sizeActions[quantActions] 
								} */
							>
								Ações
							</TableCell>
				...
				<TableBody>
					...
					{!isLoading &&
						...
						rows &&
						rows.map &&
						rows.map(({ isViewButtonHidden, ...row }, index) => (
							<TableRow
								key={v4()}
							...
							>
								...
								{onSelectRadio && (
									// sets the width to 0% so the radio button uses only the required size
									<TableCell
										component="td"
