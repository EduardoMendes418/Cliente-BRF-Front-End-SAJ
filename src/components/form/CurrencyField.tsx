...
const CurrencyField = ({ required, min, max, readOnly, validate, name, ...props }: NumProps & Props & TextFieldProps) => {

	const { initialValues, setFieldValue, status, touched } = useFormikContext<FormikContext>()

	useEffect(() => {
		const value = getNested(name, initialValues)
		if (value !== undefined && !getNested(name, touched) && value !== "") setFieldValue(name, toCurrency(value))
